...
import { attachTslUniforms, createWindNode, leafNormalNode } from './tsl';
...
export class Tree extends THREE.Group {
  ...
  update(elapsedTime) {
    const uniforms = this.leavesMesh.material?.userData?.tslUniforms;
    if (uniforms?.uTime) {
  ...
  */
  ...
  /**
   * Creates the leaf material, including the wind sway node, from
   * the current options
   * @returns {THREE.MeshStandardNodeMaterial}
   */
  #createLeafMaterial() {
    const leafMap = this.options.leaves.map ?? null;
    const mat = new MeshStandardNodeMaterial({
      name: 'leaves',
      map: leafMap,
    ...
    // Shadow passes use a separate node material, so repeat the leaf cutout
    // test there to keep transparent texels from casting rectangular shadows.
    mat.shadowNode = createAlphaShadowNode(leafMap, mat.alphaTest);

    const wind = createWindNode({
      strength: { x: 0.5, y: 0, z: 0.5 },
      frequency: 0.5,
      scale: 70,
      mode: 'leaves',
    });
    mat.positionNode = wind.positionNode;
    mat.normalNode = leafNormalNode(this.options.leaves.roundedNormals);
    attachTslUniforms(mat, wind.uniforms);
